...
5. When data is ready, run `labmate kaggle baseline <workspace>` to create a
   validated constant baseline and log it in `results.tsv`.
6. Implement model code only after metric, target, validation split, leakage
   risks, and submission format are verified.
7. Log every run in `results.tsv`.
8. Do not submit to Kaggle unless the user explicitly approves the exact file
